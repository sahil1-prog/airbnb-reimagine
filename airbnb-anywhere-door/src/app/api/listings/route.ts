          ...
          if (properties.length > 0) {
            const mappedStays = properties.slice(0, 32).map((property: any, index: number) => {
              ...
              if (property.images && property.images.length > 0) {
                const nonGoogleImg = property.images.find(
                  (img: any) => img.original_image && !img.original_image.includes("googleusercontent.com")
                );
                if (nonGoogleImg) {
                  imageUrl = nonGoogleImg.original_image;
                } else {
                  const rawImg = property.images[0].original_image || property.images[0].thumbnail || "";
                  const eqIdx = rawImg.lastIndexOf("=");
                  imageUrl = (rawImg.includes("googleusercontent.com") && eqIdx !== -1)
                    ? rawImg.substring(0, eqIdx) + "=w600-h400-n-k-no-v1"
                    : rawImg;
                }
              } else if (property.thumbnail) {
                const rawImg = property.thumbnail;
                const eqIdx = rawImg.lastIndexOf("=");
                imageUrl = (rawImg.includes("googleusercontent.com") && eqIdx !== -1)
                  ? rawImg.substring(0, eqIdx) + "=w600-h400-n-k-no-v1"
                  : rawImg;
...
Based on the location and search grounding context, generate a JSON with:
1. Generate 24-28 popular stays/hotels in this city. For each stay, map it to an appropriate category selected ONLY from: [${allowedCatsStr}]. Do NOT use "Beach" since it is not in the allowed list for this location. Aim for at least 3-4 entries per allowed category for variety. Each stay should have unique details, a realistic name, and amenities matching its assigned category.
2. Generate 8-10 popular local experiences/activities to do in this city.
3. Generate 6-8 local service listings (specifically category "Photography" or "Make-up").
